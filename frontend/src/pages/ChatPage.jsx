...
const ChatPage = () => {
  ...
  const handleStatusUpdate = (updateResponse) => {
    if (updateResponse.success) {
      setTicket(prev => ({
        ...prev,
        status: updateResponse.new_status
      }));
    }
    
    setToast({
      message: updateResponse.message,
      type: updateResponse.type || (updateResponse.success ? 'success' : 'error')
    });
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b px-4 py-3 shadow-sm">
        <div className="flex items-center gap-4 mb-3">
          ...
          <span className={`text-xs px-3 py-1 rounded-full font-medium ${getStatusBadgeColor(ticket.status)}`}>
